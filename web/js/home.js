...
function renderGrid(files) {
    files.forEach(filename => {
        ...
        delBtn.className = "card-action-btn absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 z-10";
        ...
        delBtn.onclick = async (e) => {
            e.stopPropagation();
            if (!confirm(`Delete "${filename}" and its meta/group files?`)) return;
            try {
                const res = await fetch('/mini/delete_workflow', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ filename }) });
                const data = await res.json();
                if (res.ok) { els.status.innerText = `Deleted ${data.files?.join(', ') || filename}`; await loadWorkflows(); }
